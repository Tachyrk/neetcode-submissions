...
public:
    int goodNodes(TreeNode* root) {
        ...
        int ans = 0;
        while(!q.empty()){            
            auto [node , maximum] = q.front();
            q.pop();
            if(node->val >= maximum) ans++;
            if(node->left) q.push({node->left, max(maximum, node->val)});
            if(node->right) q.push({node->right, max(maximum, node->val)});
        }
